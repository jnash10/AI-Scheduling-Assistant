...
def get_all_attendee_events(proposed_time, input_request):
    """Get all attendees' calendar events for the proposed time window."""
    events = {}
    # get sender events
...
def get_all_attendee_events_2_days(proposed_time, input_request):
    """Get all attendees' calendar events for the proposed time window's day plus the next day."""
    events = {}
    # get sender events
    from_email = input_request["From"]
    events[from_email] = get_attendee_events_2_days(proposed_time, from_email)
    # get all other attendees events
    for attendee in input_request["Attendees"]:
        user_email = attendee["email"]
        events[user_email] = get_attendee_events_2_days(proposed_time, user_email)
    return events
